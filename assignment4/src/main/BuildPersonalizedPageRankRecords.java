...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
    ...
    private static ArrayListOfIntsWritable sourceNids = new ArrayListOfIntsWritable();
    ...
    public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNode>.Context context) {
      ...
      String[] temp = context.getConfiguration().get(SOURCES).split(",");
      for (int i = 0; i < temp.length; i++) {
        if (!temp[i].isEmpty()) 
          sourceNids.add(Integer.parseInt(temp[i])); 
      }
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      int intNid = Integer.parseInt(arr[0]);

      nid.set(intNid);
      if (arr.length == 1) {
        node.setNodeId(intNid);
      ...
      // if it is a source node, then initially the pagerank value
      // is one, or the log is zero
      if ( sourceNids.contains(intNid) ) {
        node.setPersonalizedPageRank(intNid, 0);
      } else {
        // nothing to do here. the page rank is already initialized
  ...
  public int run(String[] args) throws Exception {
    ...
    //conf.setInt(SOURCES, Integer.parseInt(sourceNodes[0]));
